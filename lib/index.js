...
import { normalizeAliyunBl, normalizeKimiLocalUsage, normalizeKimiUsages } from "./quota.js";
...
let TurnCostService = (() => {
  ...
  return class TurnCostService extends _classSuper {
    ...
    kimiServerTokenFile;
    /** Harness credential store (`~/.dsh/.credentials.yaml`) for official API keys. */
    credentialsFile;
    ...
    /**
     * Kimi subscription windows. Two mutually exclusive credential paths,
     * selected by the route's `baseUrl` (or the default when unset):
     * 1) **Official coding API** (`GET {baseUrl}/usages`, default
     *    `https://api.kimi.com/coding/v1`) with a long-lived API key from the
     *    harness `.credentials.yaml` refs block (`credentialRef`, default
     *    `KIMI_CODING_API_KEY`) — works with zero local services, which is
     *    the open-the-GUI-and-it-just-works path;
     * 2) **Local Kimi Code OAuth server** (`GET {baseUrl}/api/v1/oauth/usage`,
     *    default `http://127.0.0.1:58627`) with `~/.kimi-code/server.token` —
     *    used only when the route explicitly targets a loopback baseUrl.
     * Both failures degrade to `ok:false` with a stable error code.
     */
    async fetchKimiQuota(routeCfg) {
      ...
      if (/^https:\/\//.test(baseUrl)) {
        ...
        if (key === undefined) return { kind: routeCfg.kind, ok: false, error: "kimi-credential-not-found" };
        let payload;
        try {
          payload = await fetchJson(`${baseUrl}/usages`, { headers: { Authorization: `Bearer ${key}` } });
        } catch {
          return { kind: routeCfg.kind, ok: false, error: "kimi-api-unavailable" };
        }
        const normalized = normalizeKimiUsages(payload);
      ...
      // Loopback path: the local Kimi Code OAuth server owns the session.
