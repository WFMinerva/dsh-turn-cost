...
function loadPluginFile(ratesPath, warn) {
  const loaded = { rates: builtinRates(), quota: {} };
  ...
  loaded.quota = quotaConfigOf(parsed);
  return loaded;
}
...
/**
 * Normalize `bl usage token-plan --output json` output. Verified live
 * 2026-08-24: the CLI returns `{ per1WeekPercentage, per1WeekResetTime }`
 * where percentage is a 0–1 fraction of the 7-day window used and resetTime
 * is epoch ms. We also accept the hypothetical Credits-denominated shape
 * (`total`/`used`/`remaining`) in case the CLI evolves; unrecognized output
 * degrades to ok:false with a truncated raw excerpt.
 */
function normalizeAliyunBl(payload) {
  if (payload === null || typeof payload !== "object") return null;
  // Live shape (bl 1.17.1): per-week percentage + reset timestamp.
  ...
  // Hypothetical Credits-denominated shape (future-proofing).
  const pick = (...keys) => {
