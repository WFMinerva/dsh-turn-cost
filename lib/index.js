 ...
 *
 * A Typert Remote service (`turnCost`) answering the per-turn cost
 * (`turnCost/query`), whole-session totals (`turnCost/sessionTotals`) and
 * subscription-window quota (`turnCost/quota`) from provider-reported usage
 * in the durable session logs merged with the live in-memory session events.
 * Pricing uses the built-in official DeepSeek CNY card, optionally overlaid
 * with a custom rate table (`ratesPath` config — JSON, never committed, see
 * README).
 *
 * Subscription quota (Kimi / Aliyun Token Plan) is read from the official
 * local services: the Kimi Code loopback OAuth server (`127.0.0.1:58627`)
 * and the Model Studio CLI (`bl`). When the Kimi loopback server is not
 * running on the default port, this plugin starts it itself (and shuts down
 * only the instance it started); the Aliyun `bl` command defaults to the
 * pinned private copy under `~/.dsh/turn-cost-tools` so a directly-launched
 * DSH (without the launcher script) can still read quota.
...
import { builtinQuotaRoutes, builtinRates, costOfSession, costOfTurn, findSessionFile, foldEvents, isValidSessionId, mergeQuotaRoutes, mergeRates, readSessionSamples } from "./fold.js";
...
/**
 * Pinned private-tools bin dir (`<dshHome>/turn-cost-tools/node_modules/.bin`),
 * or undefined when the install layout is absent. The launcher installs the
 * Kimi / Model Studio CLIs here pinned by lockfile; pointing at it directly
 * lets a directly-launched DSH read quota without the launcher script.
 */
function dshToolsBin() {
  try {
    const bin = join(dirname(dshHomePath("sessions")), "turn-cost-tools", "node_modules", ".bin");
...
/** Whether the Kimi loopback server answers its health endpoint. */
async function probeKimiHealth(baseUrl) {
  try {
    const body = await fetchJson(`${baseUrl}/api/v1/healthz`, { timeoutMs: 1000 });
...
/** Whether any TCP listener is bound to 127.0.0.1:port (best-effort). */
...
let TurnCostService = (() => {
  ...
  return class TurnCostService extends _classSuper {
    ...
    kimiServerTokenFile;
    /** Pinned private tools bin dir (`~/.dsh/turn-cost-tools/node_modules/.bin`) when present. */
    toolsBin;
    /** Kimi loopback child process started by this plugin (only ever shut down if we started it). */
    kimiChild = null;
    ...
    constructor(ctx, config = {}) {
      ...
      // Drop the cached folds when the plugin is reconfigured/reloaded, and
      // shut down only a Kimi loopback server this plugin started itself.
      ctx.effect(() => () => {
        this.cache = new Map();
        this.quotaCache = new Map();
        void this.stopSelfKimiServer();
      }, "dsh-turn-cost: reset caches and stop self-started Kimi server on unload");
    }
    ...
    async fetchKimiQuota(routeCfg) {
      ...
      // A directly-launched DSH (no launcher script) leaves the loopback
      // server down. Try once to start the pinned private Kimi CLI so the
      // quota readout works without the launcher; failure degrades quietly.
      try {
        await this.ensureKimiServer(baseUrl);
      } catch {
        // nothing to add — the fetch below reports the concrete error
      }
      let payload;
      try {
    ...
    /**
     * Start the Kimi Code loopback OAuth server on the default port when it
     * is not already answering, using the pinned private CLI under
     * `~/.dsh/turn-cost-tools`. Only ever starts a server we own, only on
     * the default port, and only when a valid bearer token is present; the
     * child is recorded in `kimiChild` and shut down by `stopSelfKimiServer`
     * on plugin unload. Never throws on provider failure — the caller's
     * fetch reports the concrete error instead.
     */
    async ensureKimiServer(baseUrl) {
      // Already healthy on this port? Nothing to do.
      const healthy = await probeKimiHealth(baseUrl);
      if (healthy) return;
      if (baseUrl !== "http://127.0.0.1:58627") return; // only manage the default loopback server
      if (await portInUse(58627)) return; // someone else owns the port — never kill it
      const kimiCli = this.toolsBin === undefined ? undefined : join(this.toolsBin, "kimi.cmd");
      if (kimiCli === undefined || !existsSync(kimiCli)) return;
      if (this.kimiChild !== null) return; // we already tried once this session
      // Windows npm shims are .cmd files, so run through the system command
      // processor explicitly; fixed arguments are not shell data.
      const child = spawn(
        process.platform === "win32" ? (process.env.ComSpec ?? "cmd.exe") : kimiCli,
        process.platform === "win32"
          ? ["/d", "/s", "/c", `"${kimiCli}" web --no-open --port 58627`]
          : ["web", "--no-open", "--port", "58627"],
        { windowsHide: true, stdio: "ignore" },
      );
      this.kimiChild = child;
      const deadline = Date.now() + 20_000;
      while (Date.now() < deadline) {
        if (await probeKimiHealth(baseUrl)) return;
        await new Promise((resolve) => setTimeout(resolve, 250));
      }
      // Never leave a half-dead child behind: stop what we started.
      void this.stopSelfKimiServer();
    }

    /** Shut down only a Kimi loopback server this plugin started. */
    async stopSelfKimiServer() {
      const child = this.kimiChild;
      this.kimiChild = null;
      if (child === null || child === undefined) return;
      try {
        const token = readLocalBearerToken(this.kimiServerTokenFile);
        if (token !== undefined) {
          await fetchJson("http://127.0.0.1:58627/api/v1/shutdown", {
            method: "POST",
    ...
    async fetchAliyunQuota(routeCfg) {
      const command = routeCfg.command ?? (this.toolsBin !== undefined && existsSync(join(this.toolsBin, "bl.cmd")) ? join(this.toolsBin, "bl.cmd") : "bl");
