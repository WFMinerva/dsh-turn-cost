...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"badge.title": "本轮 token 账；金额按本地费率表估算（默认隐藏，可在费率表 display.showCost 开启），不构成账单",
			"dock.line": "{models} · 本会话 {cost} · {tokens} token · 缓存读 {cache}",
			"dock.tokensOnly": "{models} · 本会话 {tokens} token · 缓存读 {cache}",
			"dock.title": "本会话累计（模型名读自对话日志，token 口径与官方统计条一致）；金额默认隐藏",
			"dock.quota": " · 5h 窗口 本会话 {requests} 次 ≈{share} · 还剩 {remaining}",
			"dock.quotaTitle": "Kimi 订阅 5 小时窗口：本会话发起的请求数占窗口上限的比例（仅 DSH 侧可归因）；还剩数来自官方 usages 端点实时读数",
			"summary.button": "额度汇总",
			...
			"quota.window.line": "{name}：已用 {used}/{limit} 次 · 还剩 {remaining} · {resetAt} 重置",
			"quota.booster": "加油包：余额 ¥{balance} · 本月已用 ¥{used} / 上限 ¥{limit}",
			"quota.aliyun.line": "7 天限额：已用 {used} / {total} Credits · 还剩 {remaining}",
		...
		const en = {
			...
			"badge.title": "Turn token accounting; money is a local rate-table estimate (hidden by default — display.showCost in the rates file), never a bill",
			"dock.line": "{models} · Session {cost} · {tokens} tokens · {cache} cached",
			"dock.tokensOnly": "{models} · Session {tokens} tokens · {cache} cached",
			"dock.title": "Whole-session totals (model names read from the conversation log; token accounting matches the shipped stats line); money hidden by default",
			"dock.quota": " · 5h window: this session {requests} req ≈{share} · {remaining} left",
			"dock.quotaTitle": "Kimi subscription 5-hour window: this session's request share of the window cap (DSH-originated calls only); remaining is the live official usages reading",
			...
			"quota.window.line": "{name}: used {used}/{limit} · {remaining} left · resets {resetAt}",
			"quota.booster": "Booster wallet: ¥{balance} left · this month ¥{used} / ¥{limit}",
			"quota.aliyun.line": "7-day quota: {used} / {total} Credits used · {remaining} left",
		...
		function TurnCostBadge({ messageId, sessionId, useSession, t, queryCost }) {
			...
			// Zero/hidden-cost turn (subscription route, or 门二 v2 金额默认隐藏
			// host 端 maskCost 置 null): show tokens only, never a misleading
			// ¥0.00 — and never hide the line itself.
		 ...
		 * moves (debounced); money renders only when the host sends it
		 * (display.showCost); every failure degrades quietly.
		 */
		function SessionDockLine({ sessionId, useProjection, t, querySessionTotals, queryQuota }) {
			...
			// Kimi-route sessions get the 5h-window share appended (门二 v2:
			// 读数行只显 5h；其余窗口在汇总面板).
			let quotaText = null;
			...
			for (const routeId of sessionProviders) {
				...
				const share5h = route.attribution?.["5h"];
				const window5h = (route.windows ?? []).find((w) => w.name === "5h");
				if (share5h === undefined || window5h === undefined) continue;
				quotaText = t("dock.quota", {
					requests: share5h.requests,
					share: formatShare(share5h.share),
					remaining: window5h.remaining ?? "--",
				});
				break;
			}
		...
		function SummaryTable({ head, rows, t, showCost = true }) {
					...
					jsx("thead", {
						children: jsxs("tr", {
							children: [
								...
								showCost ? jsx("th", { children: t("summary.col.cost") }) : null,
							],
						}),
					}),
					jsx("tbody", {
						children: rows.map((row) => jsxs("tr", {
							children: [
								...
								showCost ? jsx("td", { children: row.priced > 0 ? formatCost(row.cost) : "—" }) : null,
							],
						}, row.label)),
					}),
				],
				...
				if (route?.ok === true && route.kind === "kimi-usages") {
					for (const w of route.windows ?? []) {
						lines.push(jsx("div", {
							children: t("quota.window.line", {
								name: w.name, used: w.used, limit: w.limit,
								remaining: w.remaining ?? "--", resetAt: formatClock(w.resetAt),
							}),
						}, w.name));
					}
					...
					lines.push(jsx("div", {
						children: t("quota.aliyun.line", {
							used: route.used ?? "--", total: route.total ?? "--", remaining: route.remaining ?? "--",
						}),
					}, "aliyun"));
					if (typeof route.expireAt === "string") {
						lines.push(jsx("div", { children: t("quota.aliyun.expire", { expireAt: route.expireAt }) }, "expire"));
					}
				} else {
					lines.push(jsx("div", {
		...
		function SummaryButton({ t, querySummary, queryQuota }) {
			...
			const panelBody = () => {
				...
				const showCost = value.showCost === true;
				...
				return jsxs(react.Fragment, {
					children: [
						...
						jsx(SummaryTable, { head: t("summary.col.model"), rows: modelRows, t, showCost }),
						jsx("h4", { children: t("summary.byDay", { days: SUMMARY_DAYS }) }),
						jsx(SummaryTable, { head: t("summary.col.day"), rows: dayRows, t, showCost }),
