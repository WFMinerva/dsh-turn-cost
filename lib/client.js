...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"badge.quota": "本轮 {tokens} token · 5h 额度消耗 {share} · 剩余 {remaining}",
			"badge.quotaTitle": "Kimi 订阅：本轮的请求数占 5 小时窗口上限的比例（≈，仅 DSH 侧可归因）；剩余为官方 usages 端点实时读数",
		...
		const en = {
			...
			"badge.quota": "Turn {tokens} tokens · 5h quota −{share} · {remaining} left",
			"badge.quotaTitle": "Kimi subscription: this turn's request share of the 5-hour window cap (≈, DSH-originated only); remaining is the live official usages reading",
		...
		function TurnCostBadge({ messageId, sessionId, useSession, t, queryCost, queryQuota }) {
			...
			if (result.provider === "kimi-coding") {
				...
				if (win5h !== undefined) {
					const share = win5h.limit > 0 ? result.requests / win5h.limit : null;
					...
					return react_jsx_runtime.jsx("span", {
						...
						children: t("badge.quota", { tokens, share: formatShare(share), remaining: formatShare(remaining) }),
					});
				}
			}
			if (result.provider === "qwen-token-plan-cn") {
				const route = quota?.routes?.["qwen-token-plan-cn"];
				if (route?.ok === true && typeof route.total === "number" && typeof route.remaining === "number" && route.total > 0) {
					return react_jsx_runtime.jsx("span", {
						className: "dsh-turn-cost-badge",
						title: t("badge.qwenTitle"),
						children: t("badge.qwen", { tokens, remaining: formatShare(route.remaining / route.total) }),
					});
				}
			}
