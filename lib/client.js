...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"badge.quota": "本轮 {tokens} token · 5h 还剩 {remaining} 次",
			"badge.quotaTitle": "Kimi 订阅：5 小时窗口剩余次数为官方读数（coding API 或本地 OAuth 服务）；官方未提供可靠的单轮消耗归因，故不显示消耗百分比",
			...
			"dock.quota": " · 5h 还剩 {remaining} 次",
			"dock.quotaTitle": "Kimi 订阅 5 小时窗口：剩余次数为官方读数（coding API 或本地 OAuth 服务），不把请求次数伪装成额度消耗",
			"summary.button": "额度汇总",
			...
			"quota.window.line": "{name}：已用 {used}/{limit} 次 · 还剩 {remaining} 次 · {resetAt} 重置",
		...
		const en = {
			...
			"badge.quota": "Turn {tokens} tokens · 5h {remaining} left",
			"badge.quotaTitle": "Kimi subscription: 5-hour remaining count is the live official reading (coding API or local OAuth server); the official API does not provide reliable per-turn attribution, so no consumption share is shown",
			...
			"dock.quota": " · 5h {remaining} left",
			"dock.quotaTitle": "Kimi subscription 5-hour window: remaining count is the live official reading (coding API or local OAuth server); request counts are not presented as quota consumption",
		...
		function TurnCostBadge({ messageId, sessionId, useSession, t, queryCost, queryQuota }) {
			...
			if (result.provider === "kimi-coding") {
				...
				if (win5h !== undefined) {
					// 官方 usages 端点已给 used/remaining 原始次数；直接显余额，不再 ÷limit 换算百分比
					const remaining = win5h.remaining;
					return react_jsx_runtime.jsx("span", {
						className: "dsh-turn-cost-badge",
						title: t("badge.quotaTitle"),
						children: t("badge.quota", { tokens, remaining }),
					});
				}
			}
