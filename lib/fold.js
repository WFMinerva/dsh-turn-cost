  ...
  if (rawQuota !== null && typeof rawQuota === "object") {
    for (const key of Object.keys(rawQuota)) {
      ...
      if (entry.kind === "kimi-usages") {
        // Two mutually exclusive credential paths (one baseUrl selects one):
        // 1) official coding API — https baseUrl + `credentialRef` into the
        //    harness `.credentials.yaml` refs block (default KIMI_CODING_API_KEY);
        // 2) local Kimi Code OAuth server — loopback baseUrl + `server.token`.
        ...
        if (typeof entry.baseUrl === "string") {
          if (/^https:\/\//.test(entry.baseUrl)) route.baseUrl = entry.baseUrl;
          else if (/^http:\/\/(?:127\.0\.0\.1|localhost):\d{1,5}$/.test(entry.baseUrl)) route.baseUrl = entry.baseUrl;
        }
      }
