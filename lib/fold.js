...
/**
 * Extract the optional `display` and `quota` blocks from the rates-file
 * JSON (the same user file carries pricing + quota config). Never throws:
 * malformed blocks degrade to defaults.
 * - `display.showCost`: boolean, default **false** (门二 v2: 金额默认隐藏).
...
export function quotaConfigOf(parsed) {
  const display = { showCost: false };
  const quota = {};
  if (parsed === null || typeof parsed !== "object") return { display, quota };
  const rawDisplay = parsed.display;
  if (rawDisplay !== null && typeof rawDisplay === "object" && typeof rawDisplay.showCost === "boolean") {
    display.showCost = rawDisplay.showCost;
  }
