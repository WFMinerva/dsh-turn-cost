...
- **当前分支**：master
- **state_based_on_commit**：`5e7327e`（0.5.1 Flash 家族重定价 + 价表时间分档；**未推送**）
- **dirty**：无（工作区干净）
- **最近完成**：
...
- **下一步唯一动作**：**门三交付确认**——机主拍板后才依次执行：推送（仅凭机主明说「推送」）→ `npm publish`（触发 2FA 网页认证，须机主在能点网页的终端跑）→ 单位机安装副本更新（`npm pack` 后覆盖 `~/.dsh/profiles/web/node_modules/dsh-turn-cost` 的 `lib/`+`package.json`，**保留** profile `cordis.patch.yml` 里的 `ratesPath` 覆盖）→ 重启 dsh web（会中断当前会话）。部署后待定：单位机 `~/.dsh/turn-cost-rates.json` 里 2026-09-10 临时加的三条 flash 覆盖是否删除（内置表修好后冗余）。另：待机主安排其他机器（家用机/笔记本）部署 0.5.x；官方 `deepseek-v4-pro` 于 2026-09-14 12:00 路由到 V4.1 Flash 的切价需再走一轮维护。
- **阻塞**：无
...
- **推送状态**：0.5.0 及 CI 修复 `221a9e4` 已推送；**0.5.1 提交 `5e7327e` 尚未推送**——推送仅凭机主明说「推送」。推送后 GitHub Actions（ubuntu `node --test` + windows `maintenance.ps1 verify`）应转绿，届时以 CI 结果为准。
